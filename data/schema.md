...
시민이 만든 하천 쓰레기 표류 관측의 표준이자 인용 가능한 포맷이다. 데이터는 열려 있고(CC BY), 허브의 힘은 데이터를 가두는 데 있지 않고 **인용 표준·프로버넌스·실시간 지도·커뮤니티**에 있다. 누구나 내려받아 쓰되 인용한다.
...
> **정직 가드**: `medium=freshwater`·`link=cellular`(위성 아님)는 설계 고정값. 위치와 상태는 물리 계산 결과이며 학습 AI 예측이 아니다. 조립 트랙·모든 표류 수치는 추정(`is_estimate`).
...
NOAA Global Drifter Program 관행을 채택하여 각 물리 유닛은 방류·종료·운명(fate)·하드웨어 디렉토리 레코드를 갖는다. 그래야 트랙 통계가 "조용한 유실"로 편향되지 않는다(회수-우선 정산과 연동).
...
- 2단계 4σ 속도 QC(NOAA GDP, Hansen & Poulain 방식): 연속 fix 간 속도를 전진 및 후진 유한차분 양쪽으로 계산하여 둘 다 국지 속도 분포의 4σ를 넘으면 그 점을 bad로 플래그한다. 이 방식은 GPS 및 셀룰러 이상치 점프에 강건하다.
...
- 원시 fix 저장을 우선한다. GPS는 약 10m 규칙 측위이므로 무거운 크리깅을 생략하고 gap/error 플래그만 공개한다.

---

## 4.5 알려진 편향·물리 공변량 (관측과학 정직 경계 — 필수)

저건현 부유체는 완전한 라그랑주 추종자가 아니다(van den Bremer & Breivik 2021에 따르면 부유 쓰레기는 해류에 windage와 Stokes 효과가 겹친다). 이를 문서화하지 않으면 데이터를 물리적으로 해석할 수 없다.
...
- **검열·선택 편향(`qc.censoring`, 반드시 명명)**: (a) 셀룰러 이탈 = 외해 도달 유닛의 **우측 검열**(`right_censored`, 신호 끊긴 뒤 궤적 미관측) (b) 회수-우선 = **근안 편중**(`nearshore`) (c) 정화이벤트 기반 = **평수·호천후 과대표집**(`fair_weather`). 하천 쓰레기 수송은 실제로 홍수가 지배(상위 5% 유량, Do et al. 2026)하지만 평수 드리프터는 그 레짐을 표집하지 못한다. 따라서 데이터의 주장 범위를 "하천 내, 근안, 평수 표층 표류"로 한정하고 flux나 오염총량으로 확대 해석하지 않는다.
- QC 적용 범위 주의: 4σ 속도 QC(Hansen & Poulain 1996)는 외해 평활 및 보간 데이터를 기준으로 하므로 원시·불규칙 하천 fix에 적용하면 홍수 펄스나 좌초를 이상치로 오탐할 수 있다. 따라서 하천에서는 물리적 상한(구간 유속)과 병용하여 사용한다.

---

## 5. Calibration commons

이는 전이 가능한 과학 자산이다. 각 드리프터 형상은 하천의 특성이 아니라 물체 자체의 성질인 windage/drag 계수를 가지므로 다른 하천으로 이동하여 적용할 수 있다.
...
국제표준 상호운용: FF-ID를 **CF-NetCDF(DSG trajectory)·OGC SensorThings·EMODnet Physics**에 매핑한 crosswalk와 무의존성 exporter가 있다 — [../docs/DATA_STANDARDS_CROSSWALK.md](../docs/DATA_STANDARDS_CROSSWALK.md), [export_standards.py](export_standards.py). ★드리프터 트랙은 trajectory이며 GBIF/Darwin Core occurrence가 아니다(따라서 생물다양성 레지스트리에 등록하지 않는다).
